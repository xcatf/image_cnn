...
model = keras.models.load_model(model_dir + 'medicine_model_v1.h5')
...
model.save(model_dir + 'medicine_model_v2.h5')
with open(model_info_dir + 'model_v2_summary.txt','w') as f:
...
plt.savefig(model_info_dir + 'model_v2_acc.jpg')
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig(model_info_dir + 'model_v2_loss.jpg')

keras.utils.plot_model(model, to_file=model_info_dir + 'medicine_model.png', show_shapes=True, show_layer_names=True, rankdir='TB', expand_nested=False, dpi=96)